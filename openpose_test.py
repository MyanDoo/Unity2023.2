...
def generate_frames():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor: #추가 코드
        while True:
            # 비디오 프레임 생성 로직
            frame = capture_frame()
            ...
            # 프레임 처리를 병렬로 실행(하는 코드를 추가) #추가 코드
            future = executor.submit(generate_virtual_frame, frame)
            processed_frame = future.result()  # 작업 결과를 얻음
